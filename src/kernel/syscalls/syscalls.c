...
void* syscall_mmap(void* addr, size_t length, int prot, int flags, SYSCALL1) {
    ...
    void* virt_addr = vmm_map(addr, 0, length, prot | VM_PROT_USER, flags | VM_MAP_FORCE);
...
int syscall_win_create(const char* title, int x, int y, int width, int height) {
    ...
    vm_space_unmap(proc->vmspace, (void *)user_virt, win->buffer_size);
    vm_space_copy_mappings(proc->vmspace,
                              (uintptr_t)win->backbuffer,
                              (uintptr_t)user_virt,
                              win->buffer_size,
                              VM_PROT_READWRITE | VM_PROT_USER);
