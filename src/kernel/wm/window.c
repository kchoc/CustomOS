...
window_t* wm_create_window(uint32_t pid, const char* title, int x, int y, int width, int height) {
    ...
    win->backbuffer = (uint32_t*)vmm_map((void*)(0xC0500000 + (win->wid * 0x400000)), 0, pages_needed * 0x1000,
            VM_PROT_READWRITE | VM_PROT_NOCACHE, VM_MAP_ZERO | VM_MAP_FORCE);
    if (!win->backbuffer) {
        kfree(win);
        return NULL;
    }
...
void wm_destroy_window(uint32_t wid) {
    ...
    while (node) {
        ...
        if (win->wid == wid) {
            list_remove(node);
            if (win->backbuffer) {
                vmm_unmap(win->backbuffer, win->buffer_size);
            }
            kfree(win);
            // Mark WM as dirty to trigger screen update
            wm_dirty = true;
...
void wm_resize_window(uint32_t wid, int width, int height) {
    ...
    win->backbuffer = vmm_map((void*)(0xC0500000 + (win->wid * 0x100000)), 0, new_size,
            VM_PROT_READWRITE | VM_PROT_NOCACHE, VM_MAP_PHYS | VM_MAP_ZERO | VM_MAP_FORCE);
