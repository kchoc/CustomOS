...
typedef enum {
    ...
    TTY_IOCTL_SETCONTROLTTY =
        5409, /* set this TTY as the controlling terminal for the current process */
    TTY_IOCTL_FLUSH =
        5410, /* flush input and/or output buffers (arg is int mask: 1=input, 2=output) */
...
typedef struct tty_ops {
    ...
    void (*rx_char)(tty_t* tty, char c,
                    uint8_t flags); /* called by keyboard IRQ handler with input character */
    ssize_t (*tx_chars)(tty_t* tty, const char* buf,
                        size_t count); /* called by tty_write to send output to hardware */
    void (*set_termios)(
        tty_t*           tty,
        const termios_t* t);      /* called by ioctl to set termios settings in hardware driver */
...
typedef struct tty {
    ...
    void (*output)(tty_t* tty, const char* buf,
                   size_t count); /* helper to write output through line discipline to out_ring */
    void* output_data;            /* opaque pointer passed to output() */

    /* --- Signals --- */
    void (*raise_signal)(tty_t*       tty,
                         tty_signal_t sig); /* helper to send signal to foreground process group */
} tty_t;
