 ...
 * ========================================================================= */
static void tty_none_output(tty_t* tty, const char* buf, size_t count)
{
    (void)tty;
    (void)buf;
    (void)count;
    // No output device, just discard the data
}
...
static void tty_rx_char(tty_t* tty, char c, uint8_t flags)
{
    ...
    if (t->c_iflag & IXON) {
        if (c == t->c_cc[VSTART])
            return; // Start output (ignored for now)
        if (c == t->c_cc[VSTOP])
            return; // Stop output (ignored for now)
    }

    if (t->c_lflag & ISIG) {
        if (c == t->c_cc[VINTR]) {
            if (tty->raise_signal)
                tty->raise_signal(tty, TTY_SIG_INT);
            tty_echo(tty, c);
        ...
        if (c == t->c_cc[VQUIT]) {
            if (tty->raise_signal)
                tty->raise_signal(tty, TTY_SIG_QUIT);
            tty_echo(tty, c);
        ...
        if (c == t->c_cc[VSUSP]) {
            if (tty->raise_signal)
                tty->raise_signal(tty, TTY_SIG_SUSP);
            tty_echo(tty, c);
            return;
        }
    }
...
{
    if (!tty->output)
        return count; // no output callback, just discard silently
...
tty_ops_t default_tty_ops = {.name        = "Default TTY Line Discipline",
                             .rx_char     = tty_rx_char,
...
int tty_attach(device_t* dev)
{
    ...
    tty->output      = tty_none_output; // default to no output device
...
{
    if (!tty || !tty->tty_ops || !tty->tty_ops->rx_char)
        return;
    // SPINLOCK REQUIRED
...
int tty_read(device_t* dev, uint64_t offset, uint32_t size, uint8_t* buf)
{
    ...
    if (tty->termios.c_lflag & ICANON) {
        for (;;) {
            ...
            if (complete)
                break; // at least one complete line available
...
int tty_write(device_t* dev, uint64_t offset, uint32_t count, const uint8_t* buf)
{
    ...
    if (!tty->tty_ops || !tty->tty_ops->tx_chars)
        return -EBADF;
