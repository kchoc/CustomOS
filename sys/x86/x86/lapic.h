...
typedef struct lapic {
    ...
    lapic_register_t rrd;          // 0x0C0			Remote Read Register (for xAPIC, read returns 0; for
                                   // x2APIC, read returns data from remote APIC)
    lapic_register_t
        ldr; // 0x0D0			Logical Destination Register (for xAPIC, bits 31:24 contain the
             // logical APIC ID; for x2APIC, this register is reserved and returns 0)
    lapic_register_t
        dfr; // 0x0E0			APIC Destination Format Register (for xAPIC, bits 28:31 indicate the
             // destination mode; for x2APIC, this register is reserved and returns 0)
