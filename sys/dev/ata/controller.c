...
{
    // Check by writing and reading back from the sector count register, which is a common method to detect legacy ATA channels
    outb(ch->io_base + ATA_REG_SECTOR_COUNT, 0x55);
    ata_wait(ch->control_base);
    uint8_t value = inb(ch->io_base + ATA_REG_SECTOR_COUNT);
    if (value != 0x55) return 0; // If we can't read back the value we wrote, the channel likely doesn't exist 
...
int ata_controller_probe(device_t* dev)
{
    ...
    if (pci_dev->vendor_id == 0x8086 && (pci_dev->product_id == 0x7010 || pci_dev->product_id == 0x7111)) {
...
{
    snprintf(dev->name, sizeof(dev->name), "ata-pci-%04x:%04x", ((pci_device_t*)dev->bus_data)->vendor_id, ((pci_device_t*)dev->bus_data)->product_id);
