...
static void text_write_cell(vga_device_t *dev,
                    ...
                    char c, uint8_t attr)
{
    ...
    if (x >= VGA_TEXT_COLS || y >= VGA_TEXT_ROWS) return;
...
{
    if (!dev || !dev->open) return;
    if (x >= VGA_TEXT_COLS) x = VGA_TEXT_COLS - 1;
    if (y >= VGA_TEXT_ROWS) y = VGA_TEXT_ROWS - 1;
    dev->cursor_x = x;
    dev->cursor_y = y;
...
{
    if (!dev || !dev->open || lines <= 0) return;
...
{
    if (!dev || !dev->open || dev->mode != VGA_MODE_GRAPH_320x200) return;
...
{
    if (!dev || !dev->open || dev->mode != VGA_MODE_GRAPH_320x200) return 0;
...
{
    if (!font || font->height == 0 || font->height > 32) return -EINVAL;

    /* -- Switch sequencer to write to plane 2 only -- */
    outb(VGA_SEQ_INDEX, VGA_SEQ_RESET);       outb(VGA_SEQ_DATA, 0x01);
    outb(VGA_SEQ_INDEX, VGA_SEQ_PLANE_WRITE); outb(VGA_SEQ_DATA, 0x04);
    outb(VGA_SEQ_INDEX, VGA_SEQ_MEM_MODE);    outb(VGA_SEQ_DATA, 0x07);
    outb(VGA_SEQ_INDEX, VGA_SEQ_RESET);       outb(VGA_SEQ_DATA, 0x03);

    /* -- Graphics Controller: read plane 2, sequential (not odd/even) -- */
    outb(VGA_GC_INDEX, VGA_GC_READ_MAP);      outb(VGA_GC_DATA, 0x02);
    outb(VGA_GC_INDEX, VGA_GC_GRAPHICS_MODE); outb(VGA_GC_DATA, 0x00);
    outb(VGA_GC_INDEX, VGA_GC_MISC);          outb(VGA_GC_DATA, 0x04);
    ...
    outb(VGA_SEQ_INDEX, VGA_SEQ_RESET);       outb(VGA_SEQ_DATA, 0x01);
    outb(VGA_SEQ_INDEX, VGA_SEQ_PLANE_WRITE); outb(VGA_SEQ_DATA, 0x03);
    outb(VGA_SEQ_INDEX, VGA_SEQ_MEM_MODE);    outb(VGA_SEQ_DATA, 0x03);
    outb(VGA_SEQ_INDEX, VGA_SEQ_RESET);       outb(VGA_SEQ_DATA, 0x03);

    outb(VGA_GC_INDEX, VGA_GC_READ_MAP);      outb(VGA_GC_DATA, 0x00);
    outb(VGA_GC_INDEX, VGA_GC_GRAPHICS_MODE); outb(VGA_GC_DATA, 0x10);
    outb(VGA_GC_INDEX, VGA_GC_MISC);          outb(VGA_GC_DATA, 0x0E);
...
int vga_set_mode(vga_device_t *dev, vga_mode_t mode)
{
    ...
    case VGA_MODE_GRAPH_320x200:
        ...
        if (dev->shadow_buf) kfree(dev->shadow_buf);
...
ssize_t vga_read(device_t *dev, uint64_t offset, uint32_t size, uint8_t *buf)
{
    ...
    if (vdev->mode == VGA_MODE_TEXT_80x25) {
        ...
        const uint8_t *src = vdev->shadow_buf
                           ? vdev->shadow_buf + offset
                           : (const uint8_t *)vdev->gfx_buf + offset;
        memcpy(buf, src, size);
        return (ssize_t)size;
    }
}
...
int vga_write(device_t *dev, uint64_t offset, uint32_t count, const uint8_t *buf)
{
    ...
    if (vdev->mode == VGA_MODE_TEXT_80x25) {
        ...
        if (count >= sizeof(vga_write_header_t)) {
            ...
            if (hdr->x < vdev->width && hdr->y < vdev->height &&
                hdr->w > 0 && hdr->h > 0) {
...
done_blit:
        ...
        for (size_t i = 0; i < count; i++) {
            ...
            if (vdev->shadow_buf) vdev->shadow_buf[offset + i] = src[i];
        }
        return (ssize_t)count;
    }
}
...
int vga_ioctl(device_t *dev, int cmd, void* arg)
{
    ...
    case VGA_IOCTL_LOAD_PALETTE: {
        ...
        if (!pal) return -EFAULT;
        for (int i = 0; i < 256; i++) {
            vga_set_palette_entry(pal->entries[i].index,
                                  pal->entries[i].r,
                                  pal->entries[i].g,
                                  pal->entries[i].b);
        }
    ...
    case VGA_IOCTL_SCROLL: {
        ...
        if (lines < 0) return -EINVAL;   /* scroll-down not yet implemented */
