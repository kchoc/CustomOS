...
{
    return ((uint64_t)(mount_data->data_start_sector +
                       (cluster - 2) * mount_data->bpb.sectors_per_cluster));
}

static inline uint64_t cluster_to_fat_offset(vfat_mount_data_t* mount_data, uint32_t cluster)
{
    uint32_t fat_entry_size =
        (mount_data->fat_type == FAT_TYPE_32) ? 4 : ((mount_data->fat_type == FAT_TYPE_16) ? 2 : 1);
    return mount_data->fat_start_sector * mount_data->bpb.bytes_per_sector +
           cluster * fat_entry_size;
}

int follow_cluster_chain(device_t* dev, vfat_mount_data_t* mount_data, uint32_t start_cluster,
                         uint32_t* next_cluster)
{
    uint64_t fat_offset = cluster_to_fat_offset(mount_data, start_cluster);
    uint8_t  buffer[4]; // Max size needed for FAT32 entry
    void*    block_buffer;
    int      res = block_read(dev, fat_offset / mount_data->bpb.bytes_per_sector, &block_buffer,
                              mount_data->bpb.bytes_per_sector);
    if (res)
        return res;
    memcpy(buffer, (uint8_t*)block_buffer + (fat_offset % mount_data->bpb.bytes_per_sector), 4);
    block_release(block_buffer);
    *next_cluster =
        (mount_data->fat_type == FAT_TYPE_32)
            ? (buffer[0] | (buffer[1] << 8) | (buffer[2] << 16) | ((buffer[3] & 0x0F) << 24))
            : ((mount_data->fat_type == FAT_TYPE_16) ? (buffer[0] | (buffer[1] << 8))
                                                     : (buffer[0] | ((buffer[1] & 0x0F) << 8)));
    return 0;
}
...
int vfat_lookup(struct vnode* dvp, const char* name, int flags, vfat_lookup_result_t* out)
{
    ...
    size_t off   = 0;
    size_t bytes = dir->is_root && (mnt->fat_type == FAT_TYPE_12 || mnt->fat_type == FAT_TYPE_16)
                       ? mnt->bpb.bytes_per_sector
                       : mnt->cluster_size_bytes;
    while (1) {
        // --- read sector ---
        off     = 0;
        ...
        if (dir->is_root && (mnt->fat_type == FAT_TYPE_12 || mnt->fat_type == FAT_TYPE_16)) {
            ...
            if (cluster >= mnt->root_dir_sectors)
                goto end;
        ...
        for (off = 0; off < bytes; off += sizeof(vfat_standard_entry_t)) {
            ...
            if (entry->name[0] == 0xE5) {
                if (!(flags & VFAT_LOOKUP_FREE) || free_found)
                    continue;
                ...
                free_count++;
                if (free_count == min_free_entries)
                    free_found = true;
                continue;
            }
            ...
            check_lfn_name:
                // Check here whether the LFN data in this string matches the corresponding part of
                // the name we're looking for. If not, we can skip the rest of the LFN entries for
                // this file and
        ...
        found:
            ...
            if (flags & VFAT_LOOKUP_FREE)
                return -EEXIST; // FIND_FREE: Found an existing entry with the same name

            out->sector  = sector;
            out->offset  = off;
            out->cluster = entry->first_cluster_low | (entry->first_cluster_high << 16);
    ...
    if (flags & VFAT_LOOKUP_FIND)
        return -ENOENT; // FIND: Reached end of directory without finding the entry
    ...
    // Check if there is enough space for the new entry at the end of the directory (either in
    // current cluster or by allocating a new cluster if needed)
    if (dir->is_root && (mnt->fat_type == FAT_TYPE_12 || mnt->fat_type == FAT_TYPE_16)) {
        if (cluster < mnt->root_dir_sectors ||
            (off + min_free_entries * sizeof(vfat_standard_entry_t) <= mnt->bpb.bytes_per_sector)) {
    ...
    out->sector = cluster;
    out->offset = off;
    return 0; // FIND_FREE: Reached end of directory but there is space for new entry (either in
              // current cluster or by allocating a new cluster)
}
...
int vfat_free_cluster_chain(device_t* dev, vfat_mount_data_t* mount_data, uint32_t start_cluster)
{
    ...
    while (!IS_CLUSTER_END(mount_data, cluster)) {
        ...
        int      res = block_write(dev, fat_offset / mount_data->bpb.bytes_per_sector, empty_entry,
                                   mount_data->bpb.bytes_per_sector);
        if (res)
            return res;
...
int vfat_allocate_cluster(device_t* dev, vfat_mount_data_t* mount_data, uint32_t prev_cluster,
                          uint32_t* new_cluster)
{
    ...
    for (uint32_t cluster = 2; cluster < mount_data->total_clusters + 2; cluster++) {
        ...
        if (entry_value == 0) {
            ...
            res = block_write(dev, fat_offset / mount_data->bpb.bytes_per_sector, buffer,
                              mount_data->bpb.bytes_per_sector);
            if (res)
                return res;
            ...
            if (prev_cluster != 0) {
                ...
                res = block_read(dev, prev_fat_offset / mount_data->bpb.bytes_per_sector,
                                 &prev_block_buffer, mount_data->bpb.bytes_per_sector);
                if (res)
                    return res;

                uint8_t* prev_entry_ptr = (uint8_t*)prev_block_buffer +
                                          (prev_fat_offset % mount_data->bpb.bytes_per_sector);
                ...
                res = block_write(dev, prev_fat_offset / mount_data->bpb.bytes_per_sector,
                                  prev_block_buffer, mount_data->bpb.bytes_per_sector);
