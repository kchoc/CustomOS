...
{
    if (!mnt || !mnt->mnt_dev_vnode)
        return -EINVAL; // Invalid mount structure or device vnode
    ...
    memcpy(bpb, buffer,
           sizeof(bpb_t) + sizeof(bpb_extension_fat32_t)); // Copy the BPB data from the buffer into
                                                           // our mount data structure
    ...
    uint32_t total_sectors =
        bpb->total_sectors_short ? bpb->total_sectors_short : bpb->total_sectors_long;
    uint32_t data_sectors =
        total_sectors -
        (bpb->reserved_sector_count + (bpb->fat_count * bpb->fat_size_sectors) +
    ...
    if (total_clusters < 4085) {
        ...
        mount_data->root_cluster =
            mount_data->bpb_ext.fat32
                .root_cluster; // For FAT32, the root directory starts at a specific cluster
    }

    mount_data->fat_start_sector = bpb->reserved_sector_count;
    mount_data->fat_size_sectors = mount_data->fat_type == FAT_TYPE_32
                                       ? mount_data->bpb_ext.fat32.fat_size_32
                                       : bpb->fat_size_sectors;
    mount_data->root_dir_start_sector =
        mount_data->fat_start_sector +
        (bpb->fat_count *
         mount_data->fat_size_sectors); // Root directory starts immediately after the FAT area
    mount_data->root_dir_sectors =
        ((bpb->root_entry_count * 32) + (bpb->bytes_per_sector - 1)) /
        bpb->bytes_per_sector; // Calculate root directory size in sectors
    mount_data->data_start_sector =
        bpb->reserved_sector_count + (bpb->fat_count * mount_data->fat_size_sectors) +
        mount_data->root_dir_sectors; // Calculate the starting sector of the data region
    mount_data->cluster_size_bytes =
        bpb->bytes_per_sector *
        bpb->sectors_per_cluster; // Calculate the size of a cluster in bytes
    mount_data->total_clusters =
        data_sectors / bpb->sectors_per_cluster; // Calculate the total number of clusters available
