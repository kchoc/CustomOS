                        ...
                        uint32_t cluster, uint32_t* next_cluster)
{
    ...
    case FAT_TYPE_12: {
        ...
        if (entry_offset == 511) {
            ...
            val = ((uint8_t*)blk)[entry_offset] |
                  ((uint32_t)((uint8_t*)blk)[entry_offset + 1] << 8);
            block_release(blk);
        }
...
{
    switch (mnt->fat_type) {
    case FAT_TYPE_12: return cluster >= 0xFF8;
...
{
    return ((uint64_t)(mnt->data_start_sector +
                       (cluster - 2) * mnt->bpb.sectors_per_cluster));
}

int vfat_walk_chain(device_t* dev, vfat_mount_data_t* mnt, uint32_t start_cluster, vfat_walk_cb_t cb, void* ctx)
{
    int                status         = 0;
    ...
    if (start_cluster == 0 && mnt->fat_type != FAT_TYPE_32) {
        for (uint32_t s = (uint32_t)mnt->root_dir_start_sector;
             s < mnt->root_dir_sectors + mnt->root_dir_start_sector;
             s++) {
