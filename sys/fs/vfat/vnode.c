...
static int vfat_vnode_get(mount_t* mount, vfat_lookup_result_t* lookup_result, vnode_t** result) {
  ...
  (*result)->v_type = (lookup_result->entry.attributes & 0x10) ? VNODE_TYPE_DIRECTORY : VNODE_TYPE_FILE;
...
int vfat_vnode_read(vnode_t *node, void *buf, size_t size, size_t offset) { 
  ...
  for (uint32_t i = 0; i < clusters_to_skip; i++) {
    int res = follow_cluster_chain(node->v_mount->mnt_dev_vnode->v_data, mount_data, current_cluster, &current_cluster);
    if (res) return res;
    if (IS_CLUSTER_END(mount_data, current_cluster)) return 0; // Reached end of file before reaching the desired offset
  }
  ...
  while (bytes_read < size) {
    ...
    if (bytes_read < size) {
      res = follow_cluster_chain(node->v_mount->mnt_dev_vnode->v_data, mount_data, current_cluster, &current_cluster);
      if (res) return res;
      if (IS_CLUSTER_END(mount_data, current_cluster)) break; // Reached end of file
    }
  }
...
int vfat_vnode_write(vnode_t *node, const void *buf, size_t size, size_t offset) {
  if (size == 0) return 0;
  if (node->v_type != VNODE_TYPE_FILE) return -EISDIR; // Cannot write to directories
  vfat_node_data_t* node_data = (vfat_node_data_t*)node->v_data;
  vfat_mount_data_t* mount_data = (vfat_mount_data_t*)node->v_mount->private;
...
int vfat_vnode_create(vnode_t *dir, const char *name, vmode_t mode, vnode_t **result) {
  ...
  if (mount_data->flags & VFAT_FLAG_LFN_WRITE) {
    if (strlen(name) > 255) return -ENAMETOOLONG; // VFAT long filename limit
  } else {
    if (strlen(name) > 12) return -ENAMETOOLONG; // VFAT 8.3 filename limit
  }
  vfat_lookup_result_t lookup_result;
  ...
  if (mount_data->flags & VFAT_FLAG_LFN_WRITE) {
    ...
    vfat_standard_entry_t* entry = (vfat_standard_entry_t*)((char*)buffer + lookup_result.offset);
...
int vfat_vnode_inactive(vnode_t* node) {
  // No special handling needed for now, but this is where we would clean up any resources associated with the vnode if necessary
  return 0;
}
