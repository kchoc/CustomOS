...
typedef struct devfs_device {
    ...
    void* device; // Pointer to the actual device structure (e.g., block_device_t* or char_device_t*)
...
typedef struct devfs_device_block {
    ...
    devfs_device_t devices[DEVICES_PER_BLOCK]; // Array of 8 devices per block to reduce overhead of many small allocations
