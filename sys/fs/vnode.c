...
int vnode_get(mount_t* mount, uint64_t file_id, vnode_t** result) {
    ...
    if (res != 0 && res != -ENOENT) return res; // Return error if lookup failed for reasons other than not found
    if (res == 0) {
...
void vnode_reclaim(vnode_t* vnode) {
    ...
    while (vnode->v_mount->lazy_vnode_list) {
        if (vnode->v_mount->lazy_vnode_list == vnode) {
            vnode->v_mount->lazy_vnode_list = vnode->v_mount->lazy_vnode_list->mnt_next; // Remove from lazy vnode list
