...
int iname_lookup(const char* name, vnode_t* dir, vnode_t** result) {
    ...
    if (dir && dir->v_type != VNODE_TYPE_DIRECTORY) return -ENOTDIR;
    ...
    while (token) {
      if (current->v_mounthere) {
        // TODO: I need to handle getting the v_mount properly here as there is an unlikely case that the mount could be unmounted while we are traversing it, but I will handle that later
