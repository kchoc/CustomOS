...
int vfs_register_device(device_t* dev)
{
    int      res;
    vnode_t* dev_vnode;
    vnode_t* root_dev_vnode;

    res = dev_mount->mnt_ops->get_root(dev_mount, &root_dev_vnode);
    if (res)
        return res;

    res = root_dev_vnode->v_ops->create(root_dev_vnode, dev->name, VNODE_TYPE_BLOCK_DEVICE,
                                        &dev_vnode);
    vnode_dec_ref(root_dev_vnode);

    if (res)
        return res;

    printf("Block device %s registered successfully\n", dev->name);
...
device_t* vfs_get_device(const char* name)
{
    ...
    res = root_dev_vnode->v_ops->lookup(root_dev_vnode, name, &dev_vnode);
    if (res)
        return NULL;
...
void vfs_list_devices(void)
{
    ...
    int res = dev_mount->mnt_ops->get_root(dev_mount, &root_dev_vnode);

    int bytes = root_dev_vnode->v_ops->readdir(root_dev_vnode, buf, sizeof(buf), 0);
    if (is_errno(bytes))
        PANIC_RES("Failed to read /dev directory for block devices", bytes);
    ...
    printf("End of block device list\n");
}
...
int vfs_mount_root(const char* device_name)
{
    ...
    res = root_dev_vnode->v_ops->lookup(root_dev_vnode, device_name, &root_mnt->mnt_dev_vnode);
    if (res)
        return res;
    ...
    if (res)
        PANIC("Failed to get root vnode of root filesystem\n");

    printf("Root filesystem mounted successfully on device: %s\n", device_name);
    ...
    printf("Found /dev directory in root filesystem\n");
    ...
    dev_mount->mnt_point = dev_root_vnode;
    return 0; // Success
}
